...
class CartOrdersService {
    ...
    return _ordersCol(uid)
        ...
        .asyncMap((ordersSnap) async {
          ...
          for (final doc in ordersSnap.docs) {
            ...
            result.add(
              OrderModel.fromMap(id: doc.id, map: doc.data(), items: items),
            );
          }
          return result;
        });
  ...
  Future<OrderModel?> getOrderById(String orderId) async {
    ...
    return OrderModel.fromMap(id: doc.id, map: doc.data()!, items: items);
  }
  ...
  Future wasRecentOrderCreated({
    Duration window = const Duration(seconds: 10),
  }) async {
    final uid = _uid;
    final now = DateTime.now();
    final since = Timestamp.fromDate(now.subtract(window));
    final q = await _ordersCol(
      uid,
    ).where('createdAt', isGreaterThanOrEqualTo: since).limit(1).get();
